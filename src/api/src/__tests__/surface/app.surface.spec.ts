...
import { createLimiter } from '@/middlewares'
  ...
  const mockVerifyToken = jest.fn<unknown, unknown[]>(() => ({ id: '123' }))
  jest.mock('@/lib/jwt', () => ({
  ...
  describe('App', () => {
    ...
    describe('POST /corn/buy', () => {
      const url = '/api/corns/buy'

      it('should not allow unauthorized purchase', async () => {
        mockVerifyToken.mockImplementationOnce(() => null)
      ...
      it('should allow fist corn purchase', async () => {
        const response = await request(app).post(url).expect(200)
      ...
      it('should not allow second corn purchase', async () => {
        await request(app).post(url).expect(200)

        const response = await request(app).post(url).expect(429)
